...
function action_utilisateur 
{
    while true; do
        ...
        case $action_user_choice in
            c|C) source add_del_users.sh ;;
            g|G) source add_del_groups.sh ;;
            m|M) source ch_mdp.sh ;;
            p|P) echo "Retour au menu précédent !" ; return ;; # retourne au menu précédent
            x|X) quitter ;;
            *) clear; echo "ERREUR : vous devez taper une réponse valide !" ;;
        esac
...
function action_ordinateur 
{
    while true; do
        ...
        case $action_ordi_choice in
            m|M) source gestion_machine.sh ;;
            s|S) source gestion_systeme.sh ;;
            r|R) source gestion_repertoire.sh ;;
            p|P) echo "Retour au menu précédent !" ; return ;; # retourne au menu précédent
            x|X) quitter ;;
            *) clear; echo "ERREUR : vous devez taper une réponse valide !" ;;
        esac
...
function info_ordinateur 
{
    while true; do
        ...
        case $info_ordi_choice in
            s|S) source ./storage_mngt.sh ;;
            r|R) source ./network_config.sh ;;
            p|P) echo "Retour au menu précédent !" ; return ;; # retourne au menu précédent
            x|X) quitter ;;
